<?php
	...
	*/
	...
	function getTableList($skip_authentication = false) {
		...
		// Debugging: Check if 'Types' exists in $arrTables
		if (!isset($arrTables['Types'])) {
			error_log("Warning: 'Types' table is missing from arrTables");
		}
	
		// Skip authentication and return full table list
		if ($skip_authentication || getLoggedAdmin()) {
			return $arrTables;
		}
	
		// Check permissions and filter accessible tables
		if (is_array($arrTables)) {
			foreach ($arrTables as $tn => $tc) {
				$arrPerm = getTablePermissions($tn);
	
				// Debugging: Print table name and its permissions
				error_log("Checking permissions for: $tn -> " . json_encode($arrPerm));
	
				if (!empty($arrPerm) && isset($arrPerm[0]) && $arrPerm[0]) {
					$arrAccessTables[$tn] = $tc;
				}
			}
		}
	
		// Debugging: Log the final accessible tables list
		error_log("Final accessible tables: " . json_encode($arrAccessTables));
	...
	function getTablePermissions($tn) {
		...
		$res_group = sql("SELECT tableName, allowInsert, allowView, allowEdit, allowDelete 
						  FROM membership_grouppermissions 
						  WHERE groupID='{$groupID}'", $eo);
	
		// Fetch user-specific permissions
		$res_user = sql("SELECT tableName, allowInsert, allowView, allowEdit, allowDelete 
						 FROM membership_userpermissions 
						 WHERE LCASE(memberID)='{$memberID}'", $eo);
		...
		// Process user-specific permissions (overwrite group permissions)
		...
		// Ensure all tables have at least a default permission set
		...
		// Assign permissions or return defaults if table not found
		if (!isset($table_permissions[$tn])) {
			error_log("Warning: Table '$tn' not found in permission lists. Returning default permissions.");
			return $default_permissions; // Return default permissions instead of null
		}
	
		// Set access flag if any permission is granted
		$p = $table_permissions[$tn];
		$table_permissions[$tn]['access'] = $table_permissions[$tn][0] = ($p['insert'] || $p['view'] || $p['edit'] || $p['delete']);
	...
	function logInMember(){
		global $adminConfig; // Ensure admin config is accessible
	
		$redir = 'index.php';
	
		// Check if 'signIn' key exists before accessing it
		if (!empty($_POST['signIn'])) {
			if (!empty($_POST['username']) && !empty($_POST['password'])) {
				$username = makeSafe(strtolower($_POST['username']));
				$password = md5($_POST['password']);
	
				$isValidUser = sqlValue("SELECT COUNT(1) FROM membership_users 
										 WHERE LCASE(memberID)='$username' 
										 AND passMD5='$password' 
										 AND isApproved=1 
										 AND isBanned=0");
		...
		// Handle auto-login via cookies
		elseif ((!isset($_SESSION['memberID']) || $_SESSION['memberID'] == ($adminConfig['anonymousMember'] ?? 'guest')) 
				&& !empty($_COOKIE['SAIDE_rememberMe'])) {
			
			$chk = makeSafe($_COOKIE['SAIDE_rememberMe']);
			$username = sqlValue("SELECT memberID FROM membership_users 
								  WHERE CONVERT(MD5(CONCAT(memberID, passMD5)), CHAR)='$chk' 
								  AND isBanned=0");
