...
{
    /*
    * If the input is an array, recursively clean it.
    */
    if (is_array($str)) {
        foreach ($str as $key => $value) {
            $str[$key] = $this->xss_clean($value);
        }
        return $str; // Fix: Return after processing an array
    }
    ...
    */
    $bad_words = array(
        'javascript', 'expression', 'vbscript', 'script', 'applet', 'alert',
        'document', 'write', 'cookie', 'window', 'cmd', 'passthru', 'eval',
        'exec', 'system', 'fopen', 'fsockopen', 'file', 'file_get_contents',
        'readfile', 'unlink'
    );

    foreach ($bad_words as $word) {
        $pattern = '/\b' . preg_quote($word, '/') . '\b/i';
        $str = preg_replace($pattern, '[removed]', $str);
    }
