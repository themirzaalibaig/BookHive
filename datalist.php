...
class DataList{
	...
	// get post and get variables
		...
		$FirstRecord = $_REQUEST["FirstRecord"] ?? 1;
	...
	// handle user commands ...
		...
		elseif($Filter_x != ''){
			...
			for ($i = 1; $i <= (3 * $FiltersPerGroup); $i++) {
				if (!isset($FilterField[$i]) || $FilterField[$i] == '') {
					$FilterAnd[$i] = '';
				}
			}
			...
			*/
			// Ensure the property is defined before use
if (!isset($this->FilterPage) || empty($this->FilterPage)) {
    $this->FilterPage = 'defaultFilters.php';
}

if (!is_file($this->FilterPage)) {
    if (isset($uff) && is_file($uff)) {
        $this->FilterPage = $uff;
    } elseif (isset($gff) && is_file($gff)) {
        $this->FilterPage = $gff;
    } elseif (isset($tff) && is_file($tff)) {
        $this->FilterPage = $tff;
    }
}
	...
	// apply lookup filterers to the query
		foreach($this->filterers as $filterer => $caption){
			if(isset($_REQUEST['filterer_' . $filterer])){
				if($this->QueryWhere == '')
	...
	// set query filters
		...
		for($i = 1; $i <= (datalist_filters_count * $FiltersPerGroup); $i+=$FiltersPerGroup){ // Number of filters allowed
			...
			for ($j = 0; $j < $FiltersPerGroup; $j++) {
				...
				$filterField = isset($FilterField[$fieldIndex]) ? $FilterField[$fieldIndex] : '';
				$queryField = isset($this->QueryFieldsIndexed[$filterField]) ? $this->QueryFieldsIndexed[$filterField] : '';
				$filterOperator = isset($FilterOperator[$fieldIndex]) ? $FilterOperator[$fieldIndex] : '';
				$filterValue = isset($FilterValue[$fieldIndex]) ? $FilterValue[$fieldIndex] : '';
			
				if ($filterField !== '' && $queryField !== '' && $filterOperator !== '' && ($filterValue !== '' || strpos($filterOperator, 'empty') !== false)) {
					$GroupHasFilters = 1;
					break;
				}
			}
	...
	$FiltersCode = '';
	for ($i = 1; $i <= (datalist_filters_count * $FiltersPerGroup); $i++) { // Number of filters allowed
		$filterAnd = isset($FilterAnd[$i]) ? $FilterAnd[$i] : '';
		$filterField = isset($FilterField[$i]) ? $FilterField[$i] : '';
		$filterOperator = isset($FilterOperator[$i]) ? $FilterOperator[$i] : '';
		$filterValue = isset($FilterValue[$i]) ? $FilterValue[$i] : '';
	
		if ($i % $FiltersPerGroup == 1 && $i != 1 && $filterAnd !== '') {
			$FiltersCode .= "<input name=\"FilterAnd[$i]\" value=\"$filterAnd\" type=\"hidden\">\n";
		}
	
		if ($filterField !== '' && $filterOperator !== '' && ($filterValue !== '' || strpos($filterOperator, 'empty') !== false)) {
			if (!strstr($FiltersCode, "<input name=\"FilterAnd[{$i}]\" value=")) {
				$FiltersCode .= "<input name=\"FilterAnd[{$i}]\" value=\"{$filterAnd}\" type=\"hidden\">\n";
			}
			$FiltersCode .= "<input name=\"FilterField[{$i}]\" value=\"{$filterField}\" type=\"hidden\">\n";
			$FiltersCode .= "<input name=\"FilterOperator[{$i}]\" value=\"{$filterOperator}\" type=\"hidden\">\n";
			$FiltersCode .= "<input name=\"FilterValue[{$i}]\" value=\"" . html_attr($filterValue) . "\" type=\"hidden\">\n";
		}
	}
	...
	function validate_filters($req, $FiltersPerGroup = 4, $is_gpc = true){
		...
		/* Ensure FilterAnd is either 'and' or 'or' */
		foreach($fand as $i => $f){
		...
		/* Ensure FilterField is a positive integer */
		...
		foreach($fop as $i => $f){
			$fop[$i] = trim($f);
			if($f && !isset($GLOBALS['filter_operators'][$f])){
				$fop[$i] = '';
			}
		}
		...
		for($i = 1; $i <= (defined('datalist_filters_count') ? datalist_filters_count : 10) * $FiltersPerGroup; $i++){
			if(!isset($fand[$i]) && !isset($ffield[$i]) && !isset($fop[$i]) && !isset($fvalue[$i])) continue;
	
			if(
				(!isset($fvalue[$i]) || $fvalue[$i] == '') && 
				(!isset($fop[$i]) || !in_array($fop[$i], ['is-empty', 'is-not-empty'])) || 
				(!isset($ffield[$i]) || !$ffield[$i])
			){
				unset($fand[$i], $ffield[$i], $fop[$i], $fvalue[$i]);
			} else {
				if(!isset($fand[$i])) $fand[$i] = 'and';
				if(!isset($fop[$i])) $fop[$i] = 'equal-to';
			}
		}
	
		/* Set a default value of 'and' for FilterAnd at the beginning of each filter group if not set */
		for($i = 1; $i <= (defined('datalist_filters_count') ? datalist_filters_count : 10) * $FiltersPerGroup; $i += $FiltersPerGroup){
			if(!isset($fand[$i])) $fand[$i] = 'and';
		}
