...
include("$currDir/admin/incFunctions.php");
	...
	$postedOrderBy = array();
	$maxSortBy = 5; // Set maxSortBy if it's not already defined
	
	for ($i = 0; $i < $maxSortBy; $i++) {
		// Check if OrderByField$i exists before using it
		if (!empty($_REQUEST["OrderByField$i"])) {
			$sfi = intval($_REQUEST["OrderByField$i"]);
			$sd = (!empty($_REQUEST["OrderDir$i"]) && $_REQUEST["OrderDir$i"] === 'desc') ? 'desc' : 'asc';
	...
	}elseif(isset($_REQUEST['apply_sorting'])){
