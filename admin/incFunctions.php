<?php
	...
	*/
	...
	function get_tables_info($skip_authentication = false){
		...
		foreach ($all_tables as $tn => $ti) {
			...
			// Ensure $arrPerm is an array and contains key 0 before accessing it
			if (is_array($arrPerm) && isset($arrPerm[0]) && $arrPerm[0]) {
	...
	function makeSafe($string, $is_gpc = true) {
		if ($is_gpc) $string = stripslashes($string); // Remove unnecessary get_magic_quotes_gpc() check
	...
	function getLoggedAdmin(){
		...
		}elseif(isset($_SESSION['memberID']) && $_SESSION['memberID']==$adminConfig['adminUsername']){
	...
	*/
	...
	function getLoggedGroupID(){
		if(isset($_SESSION['memberGroupID']) && $_SESSION['memberGroupID']!=''){
	...
	function insert($tn, $set_array) {
		...
		// Ensure $set_array is an array before calling count()
		if (!is_array($set_array) || count($set_array) === 0 || empty($set)) {
			return false;
		}
