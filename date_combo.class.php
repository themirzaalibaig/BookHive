...
class DateCombo{
	...
	function GetHTML($readOnly = false) {
		...
		$date_parts = explode('-', $this->DefaultDate);
	
		// Check if we have exactly 3 parts (Year, Month, Day)
		if (count($date_parts) === 3) {
			list($xy, $xm, $xd) = $date_parts;
		} else {
			// Assign default values to prevent undefined index errors
			$xy = date('Y'); // Default to current year
			$xm = date('m'); // Default to current month
			$xd = date('d'); // Default to current day
		}
		...
		$df = $this->DateFormat; // 'myd', 'dmy', etc.
	
		// Prevent undefined array key issues
		if (isset($df[0], $df[1], $df[2])) {
		...
		// Build editable date picker UI
		$editable_date = '<table style="width: 100%;"><tr>';
		for ($i = 0; $i < 3; $i++) {
			if (!isset($df[$i])) continue; // Prevent undefined index error
